...
async def get_current_user(
    ...
    try:
        ...
        # Optimization: Try to get info directly from token claims
        login_name: Optional[str] = payload.get("login_name")
        user_name: Optional[str] = payload.get("user_name")
        role: Optional[str] = payload.get("role")
        
        if all([login_name, user_name, role]):
            return AuthUser(
                user_id=user_id,
    ...
    # Fallback to DB query if missing from token or for verification
