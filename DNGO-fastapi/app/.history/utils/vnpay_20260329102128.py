...
def _make_sign_data(obj: dict) -> str:
    ...
    return "&".join(f"{k}={str(v).strip()}" for k, v in sorted_data.items())
...
def verify_signature(obj: dict) -> bool:
    ...
    # Lọc param để tính hash: tất cả param bắt đầu bằng vnp_ trừ vnp_SecureHash
    data = {k: v for k, v in obj.items() if k.startswith("vnp_") and k != "vnp_SecureHash"}
    
    # Sắp xếp key tăng dần
    sign_data = "&".join(f"{k}={v}" for k, v in sorted(data.items()))
