from sqlalchemy import (
    ...
    ForeignKey, Date, Time, Numeric, SmallInteger,
    CheckConstraint, BigInteger          # ← thêm 2 cái này
)
...
class Wallet(Base):
    ...
    details = relationship("WalletDetail", back_populates="wallet")
...
class WalletDetail(Base):
    ...
    wallet_id = Column(String(10), ForeignKey("wallet.wallet_id", ondelete="CASCADE"), nullable=False)
    order_id = Column(String(10), ForeignKey("orders.order_id", ondelete="SET NULL"), nullable=True)
    amount = Column(Integer, nullable=False)
    transaction_type = Column(String(50), nullable=False)
    created_at = Column(DateTime, server_default=func.now())

    __table_args__ = (
        CheckConstraint("amount <> 0", name="ck_walletdetail_amount_nonzero"),
        CheckConstraint(
            "transaction_type IN ('payment','payout_seller','payout_shipper','refund','penalty')",
            name="ck_walletdetail_type"
        ),
    )

    wallet = relationship("Wallet", back_populates="details")