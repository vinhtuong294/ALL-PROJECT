...
from app.models.models import Stall, Goods, Ingredient
...
def list_products(db: Session, user_id: str, page: int = 1, limit: int = 10,
    ...
    if search:
        ...
        all_goods = query.all()
        matched_ids = [
            g.ingredient_id for g in all_goods
            if g.ingredient and search_lower in remove_accents(g.ingredient.ingredient_name.lower())
        ]
        query = query.filter(Goods.ingredient_id.in_(matched_ids))
