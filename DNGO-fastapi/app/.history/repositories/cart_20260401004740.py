...
class CartRepository:
    ...
    def checkout(db: Session, buyer_id: str, body: CheckoutBody):
        ...
        # TIME SLOT
        # =========================
        time_slot = db.query(TimeSlot).filter(TimeSlot.time_slot_id == body.time_slot_id).first()
        if not time_slot:
        ...
        order = Order(
            ...
            buyer_id=buyer_id,
            total_amount=total_amount,  # total_amount ban đầu chỉ tính hàng
        ...
        # =========================
        # LẤY ĐỊA CHỈ CHỢ TỪ BẢNG MARKET
        # =========================
        market = db.query(Market).first()  # giả sử chỉ có 1 chợ, hoặc thêm filter nếu cần
        if market is None:
            raise HTTPException(status_code=400, detail="Chưa có thông tin chợ")
        market_address = market.market_address

        # =========================
        # TÍNH KHOẢNG CÁCH VÀ PHÍ SHIP
        # =========================
        distance = calculate_distance(market_address, order.delivery_address)
        order.distance_km = distance

        shipping_fee = calculate_shipping_fee(distance or 0)  # đảm bảo không None

        # =========================
        # THÊM ORDER DETAIL CHO CÁC ITEM
        # =========================
        for i in items:
            db.add(OrderDetail(
        ...
        # =========================
        # THÊM ORDER DETAIL CHO PHÍ SHIP
        # =========================
        db.add(OrderDetail(
            order_id=order.order_id,
            ingredient_id="NLDQ01",  # mã riêng cho phí ship
            stall_id="GH0000",
            quantity_order=1,
