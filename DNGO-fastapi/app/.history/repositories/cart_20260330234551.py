...
class CartRepository:
    ...
    def checkout(db: Session, buyer_id: str, body: CheckoutBody):
        ...
        # Kiểm tra gian hàng có mở cửa không
        from app.models.models import User as UserModel
        ...
        for sid in stall_ids:
            ...
            if stall:
                ...
                if owner and owner.active_status == "dong_cua":
                    raise HTTPException(400, f"Gian hàng {stall.stall_name} đang đóng cửa, không thể đặt hàng")
        ...
        # Ghép ngày hôm nay + giờ bắt đầu khung giờ
