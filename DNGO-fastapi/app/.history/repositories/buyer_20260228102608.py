...
from app.models.models import (
    ...
    DishGroup, Dish, DishClassification, Recipe
)
...
def list_districts(
    ...
    # Search không dấu
    if search:
        search_lower = remove_accents(search.lower())
        all_districts = db.query(District).all()
        matched_ids = [
            d.district_id for d in all_districts
            if search_lower in remove_accents(d.district_name.lower())
        ]
        query = query.filter(District.district_id.in_(matched_ids))
...
def list_markets(
    ...
    if search:
        ...
        matched_ids = [
            m.market_id for m in all_markets
            if search_lower in remove_accents(m.market_name.lower()) or
               search_lower in remove_accents((m.market_address or "").lower())
        ]
        query = db.query(Market).join(District)
        if district_id:
            query = query.filter(Market.district_id == district_id)
        query = query.filter(Market.market_id.in_(matched_ids))
...
def list_stalls(
    ...
    if search:
        search_lower = remove_accents(search.lower())
        all_stalls = query.all()
        matched_ids = [
            s.stall_id for s in all_stalls
            if search_lower in remove_accents(s.stall_name.lower()) or
               search_lower in remove_accents((s.stall_location or "").lower())
        ]
        query = db.query(Stall)
        if market_id:
            query = query.filter(Stall.market_id == market_id)
        query = query.filter(Stall.stall_id.in_(matched_ids))
...
def get_stall_detail(
    ...
    # Search không dấu cho sản phẩm
    if search:
        search_lower = remove_accents(search.lower())
        all_products = product_query.all()
        matched_pairs = [
            (p.ingredient_id, p.stall_id) for p in all_products
            if p.ingredient and search_lower in remove_accents(p.ingredient.ingredient_name.lower())
        ]
        if matched_pairs:
            product_query = db.query(Goods).join(Ingredient).filter(
                Goods.stall_id == stall_id,
                Goods.ingredient_id.in_([p[0] for p in matched_pairs])
            )
        else:
            product_query = db.query(Goods).filter(False)
...
def list_ingredients(
    ...
    if search:
        ...
        matched_ids = [
            i.ingredient_id for i in all_ingredients
            if search_lower in remove_accents(i.ingredient_name.lower())
        ]
        query = query.filter(Ingredient.ingredient_id.in_(matched_ids))
...
def list_categories(
    ...
    if search:
        ...
        matched_ids = [
            c.category_id for c in all_categories
            if search_lower in remove_accents(c.category_name.lower())
        ]
        query = query.filter(Category.category_id.in_(matched_ids))
...
def list_dish_categories(
    ...
    if search:
        search_lower = remove_accents(search.lower())
        all_groups = db.query(DishGroup).all()
        matched_ids = [
            g.group_id for g in all_groups
            if search_lower in remove_accents(g.group_name.lower())
        ]
        query = query.filter(DishGroup.group_id.in_(matched_ids))
...
def list_dishes(
    ...
    if search:
        ...
        matched_ids = [
            d.dish_id for d in all_dishes
            if search_lower in remove_accents(d.dish_name.lower())
        ]
        query = db.query(Dish)
        if category_id:
            query = query.join(DishClassification).filter(
        ...
        query = query.filter(Dish.dish_id.in_(matched_ids))
