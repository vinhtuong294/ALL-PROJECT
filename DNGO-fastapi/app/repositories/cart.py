...
from app.models import Cart, CartDetail, Ingredient, Stall, Goods, Order, OrderDetail, Payment, Market, Buyer
...
class CartRepository:
    ...
    def checkout(db: Session, buyer_id: str, body: CheckoutBody, background_tasks: BackgroundTasks):
        ...
        for sid in stall_ids:
            stall = db.query(Stall).filter(Stall.stall_id == sid).first()
            if stall:
                seller = db.query(UserModel).filter(UserModel.user_id == stall.user_id).first()
                if seller:
                    # Lấy danh sách sản phẩm của gian hàng này trong đơn
                    stall_items = [i for i in items if i.stall_id == sid]
                    item_names = []
                    for si in stall_items:
                        ing = db.query(Ingredient).filter(Ingredient.ingredient_id == si.ingredient_id).first()
                        if ing:
                            item_names.append(f"{ing.ingredient_name} x{int(si.cart_quantity)}")
