...
def refund_order_details(db: Session, buyer_id: str, order_id: str, items: list):
    ...
    for item in items:
        ...
        # ── Nếu hoàn hàng → cập nhật phí ship (NLQD01) cùng đơn ──
        ship_detail = db.query(OrderDetail).filter(
            OrderDetail.order_id == order_id,
            OrderDetail.ingredient_id == "NLQD01",
        ).first()
        if ship_detail and ship_detail.detail_status != "hoan_hang":
            ship_detail.detail_status = "hoan_hang"
            ship_detail.cancel_reason = reason  # cùng lý do hoàn
            db.flush()
