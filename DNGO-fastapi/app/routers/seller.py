...
def get_orders(
    page: int = Query(1, ge=1),
    limit: int = Query(10, ge=1, le=100),
    status: Optional[str] = Query(None, description="Trạng thái đơn hàng: chua_xac_nhan, da_xac_nhan, cho_shipper, dang_giao, da_giao, cho_hoan, da_hoan, da_huy, hoan_thanh"),
    db: Session = Depends(get_db),
    current_user: AuthUser = Depends(allow("nguoi_ban"))
):
    try:
        return seller_repo.get_orders(db, current_user.user_id, order_status=status, page=page, limit=limit)
    except Exception as e:
        raise HTTPException(400, str(e))
