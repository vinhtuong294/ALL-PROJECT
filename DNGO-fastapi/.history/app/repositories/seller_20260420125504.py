...
def get_orders(db: Session, user_id: str, order_status: str = None, page: int = 1, limit: int = 10):
    """
    Lấy danh sách đơn hàng của gian hàng theo trạng thái
    
    Args:
        db: Database session
        user_id: ID người dùng (người bán)
        order_status: Trạng thái đơn hàng - nếu None lấy tất cả
                     Các giá trị: chua_xac_nhan, da_xac_nhan, cho_shipper, dang_giao, 
                                 da_giao, cho_hoan, da_hoan, da_huy, hoan_thanh
    ...
    query = (
        db.query(Order)
        .join(OrderDetail, OrderDetail.order_id == Order.order_id)
        .filter(OrderDetail.stall_id == stall_id)
    )
    
    # Nếu có chỉ định order_status, thêm filter
    if order_status:
...
def confirm_order_detail(db: Session, user_id: str, order_id: str, ingredient_id: str, action: str):
    from app.models.models import Order

    if action not in ["da_duyet", "tu_choi"]:
        raise Exception("action chỉ chấp nhận da_duyet hoặc tu_choi")
    ...
    if detail.detail_status != "cho_duyet":
        raise Exception("Đơn này đã được xử lý hoặc tự động duyệt")
    ...
    if all_approved:
        # Nếu chưa có shipper (consolidation_id = None), chuyển sang cho_shipper
        # Nếu đã có shipper, chuyển sang da_xac_nhan
        if order.consolidation_id is None:
    ...
    # Lấy danh sách sản phẩm bị từ chối để thông báo
    rejected = [d.ingredient_id for d in all_details if d.detail_status == "tu_choi"]
