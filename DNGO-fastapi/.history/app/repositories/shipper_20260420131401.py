...
def update_order_status(db: Session, shipper_id: str, order_id: str,
    ...
    if new_status == "dang_giao" and current not in ("cho_shipper", "dang_giao"):
        raise ValueError("Chỉ có thể bắt đầu giao từ trạng thái 'cho_shipper'")
    
    # Nếu chuyển sang dang_giao từ cho_shipper, kiểm tra tất cả detail đã da_lay_hang
    if new_status == "dang_giao" and current == "cho_shipper":
