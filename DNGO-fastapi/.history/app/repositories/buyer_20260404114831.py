...
def refund_order_details(db: Session, buyer_id: str, stall_id: str, order_id: str, items: list):
    # =========================
    # 1. CHECK ORDER
    # =========================
    order = db.query(Order).filter(
        Order.order_id == order_id,
        Order.stall_id == stall_id,
    ...
    for item in items:
        ...
        if not detail:
            raise Exception(f"Không tìm thấy sản phẩm {ingredient_id}")

        # ❗ không cho hoàn lại lần 2
        if detail.detail_status == "hoan_hang":
            raise Exception(f"Sản phẩm {ingredient_id} đã hoàn trước đó")

        # ❗ rule cho hoàn (bạn có thể chỉnh)
        if detail.detail_status not in ["da_duyet", "dang_giao", "da_giao"]:
    ...
    if all(d.detail_status == "hoan_hang" for d in all_details):
        order.order_status = "da_hoan"
