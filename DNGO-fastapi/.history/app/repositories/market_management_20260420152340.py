...
def list_pending_sellers(db: Session, page: int = 1, limit: int = 10, search: Optional[str] = None):
    """
    Lấy danh sách người bán chưa có gian hàng (để tạo sạp)
    - approval_status = 1 (đã duyệt)
    - Chưa có stall hoặc stall bị xóa
    """
    from app.models.models import Stall
    
    offset = (page - 1) * limit

    # Tìm những user có role = "nguoi_ban", đã duyệt (approval_status = 1)
    # nhưng chưa có gian hàng
