...
from app.models.models import User, Buyer, Shipper, Stall, Wallet
...
from app.models.models import User, Buyer, Shipper, Stall, Wallet  # ← thêm Wallet


# ── Giữ nguyên tất cả hàm generate_*() và find_*() ──────────────────────────


def generate_wallet_id() -> str:
    """Tạo wallet_id ngẫu nhiên: WL + 8 ký tự"""
    chars = string.ascii_uppercase + string.digits
    random_part = ''.join(random.choices(chars, k=8))
    return f"WL{random_part}"


def create_wallet(db: Session, owner_id: str, owner_type: str) -> Wallet:
    """Tạo ví cho owner"""
...
def create_user_with_role(
    ...
    if role == "nguoi_mua":
        ...
        db.add(new_buyer)
        create_wallet(db, owner_id=buyer_id, owner_type="buyer")  # ← thêm
        ...
        db.add(new_shipper)
        create_wallet(db, owner_id=shipper_id, owner_type="shipper")  # ← thêm
        ...
        if stall_name and market_id:
            ...
            # Nếu chưa có stall, tạo ví tạm thời với owner_id = user_id
            create_wallet(db, owner_id=user_id, owner_type="seller")
...
def get_user_profile(db: Session, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    if user.role == "nguoi_mua":
        ...
        if buyer:
            ...
            result["wallet_id"] = wallet.wallet_id if wallet else None
        ...
        if stall:
            ...
            # Lấy ví từ user_id nếu chưa có stall
            wallet = db.query(Wallet).filter(Wallet.owner_id == user_id).first()
            result["wallet_id"] = wallet.wallet_id if wallet else None

    elif user.role == "quan_ly_cho":
        from app.models.models import MarketManagement, Market
        manage_market = db.query(MarketManagement, Market).join(
            Market, Market.market_id == MarketManagement.market_id
        ).filter(MarketManagement.user_id == user_id).first()

        if manage_market:
            manage, market = manage_market
            result["manage_id"] = manage.manage_id
            result["market_id"] = manage.market_id
            result["market_name"] = market.market_name
            # quan_ly_cho không có ví
...
def get_login_history(db: Session, user_id: str, limit: int = 10):
    """Lấy danh sách lịch sử đăng nhập gần đây"""
    from app.models.models import LoginHistory
