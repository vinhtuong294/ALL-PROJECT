...
def detect_intent(message: str) -> str:
    ...
    # ✅ Kiểm tra diet_type TRƯỚC search_ingredient — "chay" là tín hiệu mạnh
    DIET_PRIORITY = ["chay", "thuần chay", "vegetarian", "vegan"]
    for kw in DIET_PRIORITY:
        if kw in text:
            return "diet_type"
...
_EXCLUDE_PATTERNS = [
    r'không\s+có\s+([\w\s]+?)(?=\s+(?:không|và|,|$)|,|$)',
    r'không\s+ăn\s+được\s+([\w\s]+?)(?=\s+(?:không|và|,|$)|,|$)',
    r'không\s+ăn\s+([\w\s]+?)(?=\s+(?:không|và|,|$)|,|$)',
    r'bị\s+dị\s+ứng\s+([\w\s]+?)(?=\s+(?:không|và|,|$)|,|$)',
    r'dị\s+ứng\s+với\s+([\w\s]+?)(?=\s+(?:không|và|,|$)|,|$)',
    r'dị\s+ứng\s+([\w\s]+?)(?=\s+(?:không|và|,|$)|,|$)',
]

_EXCLUDE_STOP_WORDS = {
    "món", "ăn", "thức", "nấu", "gì", "ngon", "và", "có", "thể", "được",
}

# Single-word animal terms — if ALL words in a captured phrase are in this set
# AND the phrase is not a known compound, treat each word as a separate term.
_STANDALONE_ANIMAL_TERMS = {
    "thịt", "bò", "heo", "lợn", "gà", "vịt", "dê", "giò", "chả",
    "cá", "tôm", "cua", "mực", "nghêu", "sò", "hàu", "trứng", "sữa",
}

# Known two-word compounds that must NOT be split
_KNOWN_COMPOUND_INGREDIENTS = {
    "thịt bò", "thịt heo", "thịt gà", "thịt vịt", "thịt dê", "thịt lợn", "thịt cừu",
    "đậu phộng", "cồi điệp", "nước mắm", "tôm sú", "tôm hùm", "tôm tươi",
    "hải sản",
}


def _split_ingredient_phrase(phrase: str) -> list[str]:
    """Split 'thịt cá' → ['thịt','cá'], but keep known compounds like 'thịt heo'."""
    if not phrase:
        return []
    if phrase in _KNOWN_COMPOUND_INGREDIENTS:
        return [phrase]
    words = phrase.split()
    if len(words) <= 1:
        return [phrase]
    if all(w in _STANDALONE_ANIMAL_TERMS for w in words):
        return words
    return [phrase]


def extract_exclude_terms(message: str) -> list[str]:
    """Trích xuất các từ cần loại trừ từ 'không có X', 'bị dị ứng X', v.v."""
    text = message.lower()
    raw_terms = []
    for pattern in _EXCLUDE_PATTERNS:
        for m in re.finditer(pattern, text):
            ...
            phrase = " ".join(w for w in words if w not in _EXCLUDE_STOP_WORDS).strip()
            if phrase and len(phrase) > 1:
                raw_terms.extend(_split_ingredient_phrase(phrase))
